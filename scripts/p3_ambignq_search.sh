...
echo "===== P3: AmbigNQ with Search ENABLED (100 steps) ====="
echo "NOTE: Previous AmbigNQ F1=0.166 with search disabled (bug)."
echo "      Gold passage diagnostic: F1=0.344. Expect significant improvement."
...
export EXPERIMENT_NAME=p3-ambignq-search-enabled
...
# AmbigNQ: search ENABLED (the only dataset that needs it)
# clarify ENABLED, alpha=0.1
...
echo ""
echo "===== AmbigNQ Search-Enabled Results ====="
echo "Compare with: v8e (search disabled) F1=0.166, gold passage F1=0.344"
grep -E "val/test_score|val/f1|val/clarify|val/search" $EXPERIMENT_NAME.log | tail -20
echo ""
