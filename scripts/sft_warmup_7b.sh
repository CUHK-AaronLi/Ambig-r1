...
echo "===== SFT Warmup: Qwen2.5-7B-Instruct (3 epochs, save per epoch) ====="
...
export EXPERIMENT_NAME=sft-warmup-7b
...
# Verify model is accessible (will download if not cached)
...
# Pick the epoch with lowest val_loss for RL training
echo ""
...
# ---------- Summary: print val_loss per epoch ----------
echo ""
...
echo "  e.g., if epoch 1 (step ~330) has lowest val_loss:"
echo "  export BASE_MODEL=\$(pwd)/verl_checkpoints/$EXPERIMENT_NAME/global_step_330"
echo ""

# Create symlink to best (default: last epoch, user should override)
