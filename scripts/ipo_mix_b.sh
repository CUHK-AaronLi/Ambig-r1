...
# ======================================================================
# Phase 1, Experiment mix-B: PACIFIC + ShARC (sqrt sampling)
#
# Purpose: Test if mixing two clarify-active datasets improves
# cross-dataset generalization over PACIFIC-only.
#
# Fixed: alpha=0.1, turn_cost=0.0 (same as v8)
# Variable: Training data (PACIFIC + ShARC mixed)
# Steps: 100 (diagnostic, not full 200)
# Eval: All 5 val datasets (checked at test_freq=20)
# ======================================================================

# Step 1: Create mixed dataset if not exists
MIXED_DIR=scripts/data_process/data/mixed_b
if [ ! -f "$MIXED_DIR/train.parquet" ]; then
    echo "Creating mixed dataset (PACIFIC + ShARC, sqrt sampling)..."
...
# Use PACIFIC val for validation (most comprehensive)
...
export EXPERIMENT_NAME=ipo-mix-b-pac-sharc
...
echo "===== Phase 1 mix-B: PACIFIC+ShARC, alpha=0.1, 100 steps ====="
...
if [ -n "$BEST_STEP" ]; then
    ...
    for dataset in pacific abgcoqa sharc situatedqa ambignq; do
        ...
        output_path=$EVAL_DIR/mix_b_on_${dataset}.json
