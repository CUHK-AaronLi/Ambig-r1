...
echo "===== Remaining xeval: mixb checkpoint x 5 datasets ====="
echo "NOTE: pacific-cont x 5 datasets already done in job 7404 (evals 1-5)"

source ~/anaconda3/bin/activate
eval "$(conda shell.bash hook)"
conda activate searchr1
cd /mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code
mkdir -p out eval_results
...
CKPT_PATH=/mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code/verl_checkpoints_diag/ipo-mix-b-pac-sharc/actor/global_step_50
...
for dataset in pacific abgcoqa sharc situatedqa ambignq; do
    ...
    experiment_name="xeval-mixb-on-${dataset}"
    val_file=${VAL_FILES[$dataset]}

    echo ""
    echo "######################################################################"
    echo "# Eval $EVAL_COUNT / 5: mixb on $dataset"
    ...
    echo ">>> $experiment_name finished at $(date)"
done

echo ""
echo "===== All remaining xeval done ====="
