...
# ======================================================================
# Phase 1, Experiment mix-C: PACIFIC + ShARC + SituatedQA (sqrt sampling)
#
# Purpose: Test if three clarify-active datasets (structure + rule +
# temporal/geographic ambiguity) further improve generalization.
#
# Fixed: alpha=0.1, turn_cost=0.0 (same as v8)
# Variable: Training data (PACIFIC + ShARC + SituatedQA mixed)
# Steps: 100 (diagnostic)
# Eval: All 5 val datasets
# ======================================================================

# Step 1: Create mixed dataset if not exists
MIXED_DIR=scripts/data_process/data/mixed_c
if [ ! -f "$MIXED_DIR/train.parquet" ]; then
    echo "Creating mixed dataset (PACIFIC + ShARC + SituatedQA, sqrt sampling)..."
...
# Use PACIFIC val for validation
export DATA_DIR=$MIXED_DIR
export VAL_FILE=scripts/data_process/data/pacific_fewshot/validation.parquet
...
export EXPERIMENT_NAME=ipo-mix-c-pac-sharc-sitqa
...
echo "===== Phase 1 mix-C: PAC+ShARC+SitQA, alpha=0.1, 100 steps ====="
echo "Training data: $DATA_DIR/train.parquet"
echo "Validation: $VAL_FILE (PACIFIC val)"
...
# Step 2: Cross-dataset evaluation on best checkpoint
echo ""
echo "===== Cross-Dataset Evaluation (mix-C) ====="
CKPT_DIR=verl_checkpoints_diag/$EXPERIMENT_NAME/actor
BEST_STEP=$(ls -d $CKPT_DIR/global_step_* 2>/dev/null | sort -t_ -k3 -n | tail -1)

if [ -n "$BEST_STEP" ]; then
    EVAL_DIR=eval_results/phase1_mix_c
    mkdir -p $EVAL_DIR
    ...
    for dataset in pacific abgcoqa sharc situatedqa ambignq; do
        ...
        output_path=$EVAL_DIR/mix_c_on_${dataset}.json
        echo "Evaluating on $dataset..."
        python3 scripts/eval_checkpoint.py \
            --model_path $BEST_STEP \
            --data_path $data_path \
