...
# ======================================================================
# Phase 3: Turn Cost Sweep
#
# Fixed: Training set from Phase 1, alpha from Phase 2
# Variable: turn_cost = {0.0, 0.005, 0.01, 0.02}
# Steps: 100 per tc value (diagnostic)
#
# NOTE: Update TRAIN_DATA and BEST_ALPHA below based on Phase 1-2 results!
# If PACIFIC-only + alpha=0.1 → already have tc data, skip this.
# ======================================================================

# === CONFIGURE THIS based on Phase 1-2 results ===
...
for TC in 0.0 0.005 0.01 0.02; do
    TC_TAG=$(echo $TC | tr '.' '')
    EXPERIMENT_NAME="tc-sweep-${DATASET_TAG}-tc${TC_TAG}"

    echo ""
    echo "======================================================================"
    echo "Turn Cost Sweep: tc=$TC, alpha=$BEST_ALPHA ($EXPERIMENT_NAME)"
    echo "======================================================================"
    ...
    echo "Finished tc=$TC at $(date)"
done

echo ""
echo "===== Turn Cost Sweep Complete ====="
echo "Compare results across:"
for TC in 0.0 0.005 0.01 0.02; do
    TC_TAG=$(echo $TC | tr '.' '')
    echo "  tc=$TC: verl_checkpoints_diag/tc-sweep-${DATASET_TAG}-tc${TC_TAG}/"
done
