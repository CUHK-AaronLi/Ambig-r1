...
echo "===== SOTA Push: PACIFIC 100-step diagnostic — Qwen3-4B ====="
...
export EXPERIMENT_NAME=sota-pacific-qwen3-4b
...
# ======================================================================
# IMPORTANT: Set this to the best SFT warmup checkpoint for Qwen3-4B
# After running sft_warmup_qwen3_4b.sh, check val_loss and pick best:
#   grep "val/loss" sft-warmup-qwen3-4b.log
# Then update the BEST_STEP below.
# ======================================================================
BEST_STEP=${SFT_STEP_Q3:-330}  # Override with: SFT_STEP_Q3=660 sbatch ...
export BASE_MODEL=/mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code/verl_checkpoints/sft-warmup-qwen3-4b/global_step_${BEST_STEP}

# Verify SFT checkpoint exists
if [ ! -d "$BASE_MODEL" ]; then
    echo "ERROR: SFT checkpoint not found at $BASE_MODEL"
    echo "Available checkpoints:"
    ls -d verl_checkpoints/sft-warmup-qwen3-4b/global_step_* 2>/dev/null
    echo "Set SFT_STEP_Q3=<step> and retry."
    exit 1
fi
