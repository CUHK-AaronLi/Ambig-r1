...
echo "===== SOTA Push: PACIFIC 100-step diagnostic — Qwen2.5-7B-Instruct ====="
...
export EXPERIMENT_NAME=sota-pacific-7b
...
# ======================================================================
# IMPORTANT: Set this to the best SFT warmup checkpoint for 7B
# After running sft_warmup_7b.sh, check val_loss and pick best:
#   grep "val/loss" sft-warmup-7b.log
# Then update the BEST_STEP below.
# ======================================================================
BEST_STEP=${SFT_STEP_7B:-330}  # Override with: SFT_STEP_7B=660 sbatch ...
export BASE_MODEL=/mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code/verl_checkpoints/sft-warmup-7b/global_step_${BEST_STEP}

# Verify SFT checkpoint exists
if [ ! -d "$BASE_MODEL" ]; then
    echo "ERROR: SFT checkpoint not found at $BASE_MODEL"
    echo "Available checkpoints:"
    ls -d verl_checkpoints/sft-warmup-7b/global_step_* 2>/dev/null
    echo "Set SFT_STEP_7B=<step> and retry."
    exit 1
fi
echo "SFT checkpoint: $BASE_MODEL"

# 7B needs reduced batch sizes for memory
...
grep -E "val/test_score|val/f1|val/clarify" $EXPERIMENT_NAME.log | tail -20
