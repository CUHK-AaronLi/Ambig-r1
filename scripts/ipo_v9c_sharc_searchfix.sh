...
export DATA_DIR=scripts/data_process/data/sharc_fewshot
...
export EXPERIMENT_NAME=ipo-v9c-sharc-searchfix
...
echo "===== IPO v9c: ShARC with SEARCH FIX + turn_cost=0.02 ====="
echo "Purpose: Re-train ShARC after fixing search bug + turn_cost to prevent over-clarification"
echo "Change: (1) search actions receive real retrieval, (2) turn_cost=0.02 to prevent U-shape regression"
echo "Expected: F1 ≥ 0.719 with controlled clarification (avoid F1 drop from over-clarification)"
