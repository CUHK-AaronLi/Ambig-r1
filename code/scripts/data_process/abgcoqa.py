...
"""
Preprocess the Abg-CoQA dataset to parquet format.
"""
...
Use the passage and the conversation history to answer the current user question. \
You must conduct reasoning inside <think> and </think> every time you gather information. \
After reasoning, choose exactly one action:
1. If the current question is ambiguous or underspecified, use <clarify> clarification question </clarify>.
2. When you are confident, reply with the final answer inside <answer> and </answer>.
...
def convert_example(
    ...
    # 调试：检查 clarification_turn 是否有 question 字段
    if clarification_answers and not clarification_turn_question:
        print(f"[WARNING] Example {example.get('id', 'N/A')}: clarification_answers exist but clarification_turn.question is empty!")
        print(f"  clarification_turn keys: {list(clarification_turn.keys())}")
        print(f"  clarification_turn: {clarification_turn}")
        print(f"  clarification_answers count: {len(clarification_answers)}")
    ...
    def _variant_extra_info(
        ...
        extra = {
            ...
            "data_source": data_source,  # 添加到 extra_info 中作为回退
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--local_dir", default="./data/abgcoqa")
