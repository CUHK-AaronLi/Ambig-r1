#!/bin/bash
#SBATCH --job-name="mix5ds-100-xeval"
#SBATCH --account=pgs
#SBATCH --qos=low
...
# Evaluate mix5ds-stage2 step 100 checkpoint
export BASE_MODEL=/mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code/verl_checkpoints_diag/mix5ds-stage2/actor/global_step_100
...
echo "Evaluating: mix5ds-stage2 step 100"
...
for DS in pacific_fewshot abgcoqa ambignq_fewshot sharc_fewshot situatedqa_fewshot; do
    DS_DIR=scripts/data_process/data/$DS
    # Use test.parquet for datasets that don't have validation.parquet
    VAL_FILE=$DS_DIR/validation.parquet
    if [ ! -f "$VAL_FILE" ]; then
        VAL_FILE=$DS_DIR/test.parquet
    fi
    EXPNAME=mix5ds-100-${DS}
    ...
    echo "===== $DS Results ====="
    grep -E "val/f1|val/post_clarify|val/clarify_rate|val/no_clarify" ${EXPNAME}.log | tail -10
done

echo ""
echo "===== All 5-Dataset Eval Complete ====="
