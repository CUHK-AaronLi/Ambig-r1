...
echo "===== Post-hoc Eval: igt=0.20 best checkpoint ====="
echo "Purpose: Compute Post-Clarify F1 + ambiguity-stratified metrics"
echo "Uses updated ray_trainer.py with new metrics"

# Find the best checkpoint from igt=0.20 run
CKPT_BASE=/mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code/verl_checkpoints_diag/abl-v8a-igt020
echo "Available checkpoints:"
ls -d $CKPT_BASE/actor/global_step_* 2>/dev/null || echo "No checkpoints found"

# Use the latest checkpoint (or specify manually)
BEST_CKPT=$(ls -d $CKPT_BASE/actor/global_step_* 2>/dev/null | sort -t_ -k3 -n | tail -1)
if [ -z "$BEST_CKPT" ]; then
...
export EXPERIMENT_NAME=eval-igt020-postclr
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo_ipo \
    ...
    actor_rollout_ref.model.path=$BEST_CKPT \
...
echo ""
echo "===== Post-Clarify F1 Eval Results ====="
grep -E "val/f1|val/post_clarify|val/no_clarify|val/f1_ambiguous|val/clarify_rate_ambig" $EXPERIMENT_NAME.log | tail -20
echo ""
