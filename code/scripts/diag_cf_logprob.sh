...
#SBATCH --time=6:00:00
#SBATCH --gpus=4
#SBATCH --nodelist=CPIIGPU-211-127
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
echo "===== DIAG: Counterfactual Logprob IG on PACIFIC (20 steps) ====="
echo "Config: exact same as p5-pac-arew (which works), minus AReW, plus counterfactual_logprob"
...
export EXPERIMENT_NAME=diag-cf-logprob
...
# Exact same config as p5-pac-arew (proven working), only changes:
#   - Removed +arew.enable/lambda_as/lambda_multi
#   - Added +ipo.counterfactual_logprob=true
...
grep -E "Counterfactual-IG|val/test_score|val/f1|val/clarify|IPOv3.*counterfactual" $EXPERIMENT_NAME.log | tail -30
echo ""
