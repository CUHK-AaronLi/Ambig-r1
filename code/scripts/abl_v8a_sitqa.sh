...
export DATA_DIR=scripts/data_process/data/situatedqa_fewshot
...
export EXPERIMENT_NAME=abl-v8a-sitqa
...
echo "===== SituatedQA + Ablation IG + v8a Config (200 steps, 8 GPU) ====="
echo "Dataset: SituatedQA (9.6K, 42.7% ambiguous: temporal + geographical)"
echo "Reward: ablation IG, NO threshold, NO turn_cost (encourage clarify)"
echo "First time: ablation IG + v8a optimal config on SituatedQA"
echo "Early kill: step30 F1<0.15 or clr<5%, step60 F1<0.25 or clr<10%"
...
echo ""
echo "===== SituatedQA Ablation v8a Results ====="
