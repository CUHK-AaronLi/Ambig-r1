...
hostname
echo "===== ATCC Training: Ambiguity-Type Conditioned Clarification ====="
...
echo "Reward: ATCC (Ambiguity-Type Conditioned Clarification)"
echo "Key change: clarify_bonus varies by ambiguity type (PARAMETER=0.15, FACTUAL=0.0)"
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo_ipo \
    ...
    +ipo.dcr_mode=true \
    +ipo.atcc_mode=true \
    trainer.experiment_name=atcc-5ds \
    trainer.default_local_dir=/mnt/users_home/cpii.local/yli/Ambig-R1-new-claude/code/verl_checkpoints_diag/atcc-5ds \
    2>&1 | tee atcc-5ds.log

echo ""
echo "===== ATCC Training Complete ====="
echo "Done at: $(date)"
