...
export EXPERIMENT_NAME=abl-7b-pacific-sft
...
echo "===== RL from PACIFIC-enriched SFT — 7B, 200 steps ====="
echo "Model: Qwen2.5-7B (PACIFIC SFT warmup best checkpoint)"
echo "Reward: ablation IG with P0 fixes + No threshold (ig_threshold=0.0)"
echo "Config: alpha=0.1, micro_batch=8, 8 GPU"

# Verify checkpoint
if [ ! -d "$BASE_MODEL" ]; then
    echo "ERROR: SFT checkpoint not found at $BASE_MODEL"
    ls -d verl_checkpoints/sft-7b-pacific/global_step_* 2>/dev/null
    exit 1
fi
...
echo ""
echo "===== RL from PACIFIC SFT Results ====="
