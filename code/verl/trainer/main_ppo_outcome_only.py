"""
Outcome-Only Reward Manager — Pure F1 baseline.

Only the final F1 score is used as reward. No clarify bonus, no confidence,
no consistency. Known to cause collapse in multi-turn settings; serves as
...
class OutcomeOnlyRewardManager(BaseRewardManager):
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(batch_size):
            ...
            # Place F1 reward at last valid token only
...
def log_main(message):
    print(f"[Main-OutcomeOnly] {message}")
