...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(batch_size):
            ...
            if has_log_probs and valid_response_length > 0 and self.w['confidence'] != 0:
                confidence = self._compute_confidence(data, i, prompt_length, valid_response_length, response_str)
        ...
        # ---- Pass 2: 同组一致性 (consistency, 用 uid 分组而非位置) ----
    ...
    # ------------------------------------------------------------------
    # clarify_cnt = 本条 trajectory 里「做了 clarify」的 turn 数（即 <clarify> 出现次数）
    # 下面常数是「每 1 次 clarify（每 1 个这样的 turn）」的奖惩
    CLARIFY_RIGHT_BONUS_PER_TURN = 0.15   # clarify 且答对：每个 clarify turn +0.15
    CLARIFY_WRONG_PENALTY_PER_TURN = 0.0  # clarify 但答错：不罚，避免因怕罚而不敢问（曾用 0.08）
    NO_CLARIFY_CORRECT_BONUS = 0.2        # 没 clarify 且答对：效率 bonus

    @staticmethod
    def _adaptive_clarify_reward(f1: float, clarify_cnt: int) -> float:
        """
        根据最终答案质量决定 clarify 的奖惩（答错不罚 clarify，只少拿 F1）：
        - 答对了 (F1>=0.5) 且 clarify 过 → 给正向奖励
        - 答错了且 clarify 过 → 0（不罚，与「不 clarify 答错」一致）
        - 没 clarify 且答对了 → 效率 bonus
        - 没 clarify 且答错了 → 中性
        """
        if clarify_cnt == 0:
            return RewardManager.NO_CLARIFY_CORRECT_BONUS if f1 >= 0.5 else 0.0
        if f1 >= 0.5:
            return RewardManager.CLARIFY_RIGHT_BONUS_PER_TURN * min(clarify_cnt, 3)
        else:
            return -RewardManager.CLARIFY_WRONG_PENALTY_PER_TURN * min(clarify_cnt, 4)

    # ------------------------------------------------------------------
    # Component 2: Confidence (answer token 平均 log-prob)
    # ------------------------------------------------------------------
    def _compute_confidence(self, data: DataProto, idx: int, prompt_length: int,
                            valid_response_length: int, response_str: str) -> float:
        """
        仅用 <answer>...</answer> 内部 token 的平均 log-prob 作为 confidence。
        排除 think/clarify 等无关 token 的干扰。
        归一化到 [0, 1]（sigmoid 映射）。
        """
    ...
    def _compute_consistency(self, per_sample: List[dict]) -> List[float]:
        ...
        注意: _balance_batch 会打乱 batch 顺序，因此必须用 uid 分组，
        不能依赖位置连续性。
        ...
        for uid, indices in uid_to_indices.items():
            ...
            answers = [self._normalize_text(per_sample[i]['answer_pred']) for i in indices]
            answer_counts = Counter(answers)
            majority_count = answer_counts.most_common(1)[0][1] if answer_counts else 0
            agreement = majority_count / group_size

            for idx in indices:
                did_clarify = per_sample[idx]['clarify_cnt'] > 0
...
def main_task(config):
    ...
    if hasattr(config, 'reward_weights'):
        from omegaconf import OmegaConf
        reward_weights = OmegaConf.to_container(config.reward_weights, resolve=True)
    print(f"[Reward] weights: {reward_weights or 'defaults'}, n_agent={n_agent}")
    ...
    val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=2,
                                  ...
                                  reward_weights={'confidence': 0, 'consistency': 0,
                                                  'adaptive_clarify': reward_weights.get('adaptive_clarify', 0.3)})
