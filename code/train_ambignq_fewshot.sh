...
## AmbigQA Few-Shot + Search + Clarify
#SBATCH --job-name="ambignq-fewshot"
#SBATCH --account=pgs
#SBATCH --qos=low
...
echo "===== AmbigQA Few-Shot + Search + Clarify ====="
...
export EXPERIMENT_NAME=ambignq-fewshot
...
# ---------- Retrieval Server (login node) ----------
RETRIEVAL_URL="http://10.10.211.118:8000/retrieve"
echo "Using Retrieval Server at ${RETRIEVAL_URL} (login node)"
echo "Checking retrieval server connectivity..."
if python3 -c "import urllib.request; urllib.request.urlopen('http://10.10.211.118:8000/docs', timeout=5)" > /dev/null 2>&1; then
    echo "Retrieval server is reachable!"
else
    echo "WARNING: Retrieval server is NOT reachable! Search actions will fail."
fi

# ---------- GPT Simulator (login node) ----------
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    +reward_weights.consistency=0.0 \
